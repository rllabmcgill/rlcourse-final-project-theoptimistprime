    ...
    for i in range(n):
        ...
        while (featureIndex/20!=location[destination]):
            if np.random.random()>eps:
                action=np.argmax(policy[featureIndex])
            else:
            ...
            """
            if np.random.random()>eps:
                nextaction=np.argmax(policy[newfeatureIndex])
            else:
                nextaction=np.random.randint(possibleActions)
            update = alpha*(-1 +gamma*policy[newfeatureIndex, nextaction] - policy[featureIndex,action])
            policy += update*e
            featureIndex=newfeatureIndex
            
    policy = np.exp(policy/temperature)
    return policy/np.sum(policy, axis=1).reshape(policy.shape[0],1)
...
if __name__=="__main__":
    ...
    rOpt = np.zeros((len(options), 500))
    pOpt = np.zeros((len(options), 500, 500))
    
    if os.path.isfile('Data/Taxi/rmodel.npy'):
        rOpt = np.load('Data/Taxi/rmodel.npy')
        pOpt = np.load('Data/Taxi/pmodel.npy')    
    ...
    for o in range(len(options)):
        ...
        for i in range(500000):
            reward = 0
            t = 0
            startf = getFeatures(env.reset())
            f = startf
            done = False
            while not done:
                s,r,done = env.step(options[o].policy(f))
                f = getFeatures(s)
                done = done or options[o].terminate(f)
                reward += gamma**t * r
                t+=1
            deltaR = (reward - np.dot(rOpt[o],startf))
            rOpt[o] += alpha*deltaR*startf
            #print getState(startf),":", np.nonzero(startf), getState(f), ":", np.nonzero(f)
            #print (f-np.dot(pOpt[o],startf))
            #print 
            pOpt[o] += alpha*(gamma**t)*np.outer(startf,(f-np.dot(startf, pOpt[o])))
            if i%5000==0:
                print i

    np.save('Data/Taxi/rmodel', rOpt)
    np.save('Data/Taxi/pmodel', pOpt)
